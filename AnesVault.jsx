...
// ─── Resource API factory (used for both Documents and Education) ─────────
function makeResourceAPI(table, bucket) {
  return {
    async getApproved() {
      const { data, error } = await supabase.from(table).select("*").eq("approved", true).order("created_at", { ascending: false });
    ...
    async getPending(password) {
      const res = await fetch("/api/admin-documents", {
        ...
        body: JSON.stringify({ action: "list", password, table }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Could not load pending items");
    ...
    async verifyAdmin(password) {
      const res = await fetch("/api/admin-documents", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "verify", password, table }),
      });
      return res.ok;
    },
    async approveDoc(id, password) {
      const res = await fetch("/api/admin-documents", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "approve", id, password, table }),
      });
    ...
    async rejectDoc(id, password) {
      const res = await fetch("/api/admin-documents", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "reject", id, password, table }),
      });
...
const docAPI = makeResourceAPI("documents", "documents");
const eduAPI = makeResourceAPI("education", "education");
...
function AnesVault() {
  ...
  const [eduPending, setEduPending] = useState([]);
  const [eduLoading, setEduLoading] = useState(true);
  const [eduForm, setEduForm] = useState({ title: "", description: "", uploader: "", anon: false, file: null, agreed: false });
  ...
  const refreshEduDocs = async () => {
    setEduLoading(true);
    const data = await eduAPI.getApproved();
    setEduDocs(data);
    setEduLoading(false);
  };
  useEffect(() => { refreshEduDocs(); }, []);

  const refreshEduPending = async () => {
    try {
      const data = await eduAPI.getPending(adminPw);
      setEduPending(data);
    } catch {
      setToast("❌ Could not load pending resources.");
    }
  };
  useEffect(() => { if (view === "admin" && adminAuth) { refreshPending(); refreshEduPending(); } }, [view, adminAuth]);
  ...
  const handleDeletePublished = async (id, title) => {
    if (!window.confirm(`Permanently delete "${title}"? This can't be undone.`)) return;
    try {
      await docAPI.rejectDoc(id, adminPw); // same secure delete endpoint, works on any document
  ...
  // ── Educational Resources (Supabase) ──
  const handleEduSubmit = async () => {
    if (!eduForm.title || !eduForm.file || !eduForm.agreed) return;
    setEduUploading(true);
    try {
      const uploaded = await eduAPI.uploadFile(eduForm.file);
      await eduAPI.insert({
        title: eduForm.title,
        description: eduForm.description || "Community-submitted educational resource.",
        uploader: eduForm.anon ? "Anonymous" : (eduForm.uploader || "Anonymous"),
        file_url: uploaded.file_url,
        file_name: uploaded.file_name,
        approved: false,
      });
      setEduForm({ title: "", description: "", uploader: "", anon: false, file: null, agreed: false });
  ...
  const handleEduDownload = async (doc) => {
  ...
  const handleEduApprove = async (id) => {
    try {
      await eduAPI.approveDoc(id, adminPw);
      setEduPending(prev => prev.filter(d => d.id !== id));
      await refreshEduDocs();
  ...
  const handleEduReject = async (id) => {
    try {
      await eduAPI.rejectDoc(id, adminPw);
      setEduPending(prev => prev.filter(d => d.id !== id));
      setToast("🗑 Resource rejected.");
    } catch {
      setToast("❌ Could not reject — try again.");
    }
  };

  const handleDeletePublishedEdu = async (id, title) => {
    if (!window.confirm(`Permanently delete "${title}"? This can't be undone.`)) return;
    try {
      await eduAPI.rejectDoc(id, adminPw);
      setEduDocs(prev => prev.filter(d => d.id !== id));
  ...
  return (
    <div className="hub-root">
      ...
      {view === "education" && (
        <div className="section-wrap">
          <div className="section-head">
            <span className="section-title">Educational Resources</span>
            <button className="dl-btn" onClick={() => setShowEduForm(v => !v)}>
              <Icon name="upload" size={14} /> {showEduForm ? "Cancel" : "Suggest a Resource"}
            </button>
          </div>
          ...
          {showEduForm && (
            <div className="form-card" style={{ marginBottom: 24 }}>
              <div className="form-group">
                ...
                <input className="form-input" placeholder="e.g. Regional Anesthesia Landmark Guide" value={eduForm.title} onChange={e => setEduForm(f => ({ ...f, title: e.target.value }))} />
              </div>
              <div className="form-group">
                <label className="form-label">Why is this helpful?</label>
                <textarea className="form-textarea" placeholder="A quick note on what this covers and who it's for" value={eduForm.description} onChange={e => setEduForm(f => ({ ...f, description: e.target.value }))} />
              </div>
              <div className="form-group">
                <label className="form-label">Your Name / Handle</label>
                <input className="form-input" placeholder="e.g. CRNA_Texas or your Instagram handle" value={eduForm.uploader} onChange={e => setEduForm(f => ({ ...f, uploader: e.target.value }))} disabled={eduForm.anon} />
                <div className="anon-row" style={{ marginTop: 8 }}>
                  <input type="checkbox" id="edu-anon" checked={eduForm.anon} onChange={e => setEduForm(f => ({ ...f, anon: e.target.checked }))} />
                  <label className="anon-label" htmlFor="edu-anon">Post anonymously</label>
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Upload File *</label>
                <div className="file-drop" onClick={() => document.getElementById("edu-file-input").click()}>
                  <div className="file-drop-icon"><Icon name="upload" size={32} /></div>
                  <div className="file-drop-text">{eduForm.file ? eduForm.file.name : "Tap to choose a file"}</div>
                  <div className="file-drop-sub">PDF or image (JPG, PNG) · Max 10 MB</div>
                  {eduForm.file && <div className="file-selected">✓ {eduForm.file.name}</div>}
                  <input id="edu-file-input" type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: "none" }} onChange={e => setEduForm(f => ({ ...f, file: e.target.files[0] }))} />
                </div>
              </div>
              <div className="checkbox-row">
                <input type="checkbox" id="edu-agree" checked={eduForm.agreed} onChange={e => setEduForm(f => ({ ...f, agreed: e.target.checked }))} />
                <label className="checkbox-label" htmlFor="edu-agree">
                  I confirm I have the right to share this resource and it <strong>contains no patient PHI</strong> (Protected Health Information) or identifiable patient data.
                </label>
              </div>
              <button className="submit-btn" disabled={!eduForm.title || !eduForm.file || !eduForm.agreed || eduUploading} onClick={handleEduSubmit}>
                <Icon name="upload" size={18} /> {eduUploading ? "Uploading…" : "Submit for Review"}
              </button>
            </div>
          )}
          ...
          {eduLoading ? (
            ...
            <div className="empty-state">
              ...
              <div className="empty-text">No resources yet</div>
              <div className="empty-sub">Found a study guide or reference worth sharing? <span style={{ color: S.teal, cursor: "pointer", fontWeight: 600 }} onClick={() => setShowEduForm(true)}>Suggest one</span></div>
            </div>
          ) : eduDocs.map(doc => (
            <div key={doc.id} className="doc-card">
              <div className="doc-top">
                <div className="doc-icon"><Icon name="book" size={20} /></div>
                <div className="doc-info">
                  <div className="doc-title">{doc.title}</div>
                  <div className="doc-desc">{doc.description}</div>
                  <div className="doc-meta">
              ...
              <div className="doc-footer">
                <span />
                <div style={{ display: "flex", gap: 8 }}>
                  <a className="dl-btn" style={{ background: S.mist, color: S.teal, border: `1.5px solid ${S.border}`, textDecoration: "none" }} href={doc.file_url} target="_blank" rel="noopener noreferrer"><Icon name="search" size={14} /> View</a>
                  <button className="dl-btn" onClick={() => handleEduDownload(doc)}><Icon name="download" size={14} /> Download</button>
                </div>
              </div>
            </div>
          ))}
          ...
          {!adminAuth ? (
            ...
            <>
              ...
              <div className="section-head" style={{ marginTop: 36 }}>
                <span className="section-title">Published Documents</span>
                <span className="section-badge">{docs.length} live</span>
              </div>
              {docs.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-icon"><Icon name="file" size={48} /></div>
                  <div className="empty-text">No documents published yet</div>
                </div>
              ) : docs.map(doc => (
                <div key={doc.id} className="pending-card">
                  <div className="doc-title">{doc.title}</div>
                  <div style={{ marginTop: 4, marginBottom: 8 }}>
                    <span className="tag tag-cat">{doc.category}</span>{" "}
                    <span className="tag tag-user">↑ {doc.uploader}</span>{" "}
                    <span className="tag tag-date">{doc.downloads || 0} downloads</span>
                  </div>
                  <div style={{ fontSize: 13, color: S.slate }}>{doc.description}</div>
                  {doc.file_url && <div style={{ fontSize: 12, color: S.teal, marginTop: 6 }}><a href={doc.file_url} target="_blank" rel="noopener noreferrer">Preview file ↗</a></div>}
                  <div className="admin-actions">
                    <button className="reject-btn" onClick={() => handleDeletePublished(doc.id, doc.title)}><Icon name="x" size={14} /> Delete</button>
                  </div>
                </div>
              ))}

              <div className="section-head" style={{ marginTop: 36 }}>
                <span className="section-title">Pending Educational Resources</span>
                <span className="section-badge">{eduPending.length} pending</span>
              </div>
              {eduPending.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-icon"><Icon name="check" size={48} /></div>
                  <div className="empty-text">All caught up!</div>
                  <div className="empty-sub">No resources pending review.</div>
                </div>
              ) : eduPending.map(doc => (
                <div key={doc.id} className="pending-card">
                  <div className="doc-title">{doc.title}</div>
                  ...
                  {doc.file_url && <div style={{ fontSize: 12, color: S.teal, marginTop: 6 }}><a href={doc.file_url} target="_blank" rel="noopener noreferrer">Preview file ↗</a></div>}
                  <div className="admin-actions">
                    <button className="approve-btn" onClick={() => handleEduApprove(doc.id)}><Icon name="check" size={14} /> Approve</button>
                    <button className="reject-btn" onClick={() => handleEduReject(doc.id)}><Icon name="x" size={14} /> Reject</button>
                  </div>
                </div>
              ))}

              <div className="section-head" style={{ marginTop: 36 }}>
                <span className="section-title">Published Educational Resources</span>
                <span className="section-badge">{eduDocs.length} live</span>
              </div>
              {eduDocs.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-icon"><Icon name="book" size={48} /></div>
                  <div className="empty-text">No resources published yet</div>
                </div>
              ) : eduDocs.map(doc => (
                <div key={doc.id} className="pending-card">
                  <div className="doc-title">{doc.title}</div>
                  ...
                  {doc.file_url && <div style={{ fontSize: 12, color: S.teal, marginTop: 6 }}><a href={doc.file_url} target="_blank" rel="noopener noreferrer">Preview file ↗</a></div>}
                  <div className="admin-actions">
                    <button className="reject-btn" onClick={() => handleDeletePublishedEdu(doc.id, doc.title)}><Icon name="x" size={14} /> Delete</button>
                  </div>
                </div>
              ))}
            </>
