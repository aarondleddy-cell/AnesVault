...
function AnesVault() {
  ...
  useEffect(() => {
    if (!previewDoc || !previewDoc.fileData) { setPreviewUrl(null); return; }
    let url = null;
    try {
      const parts = previewDoc.fileData.split(",");
      const mime = (parts[0].match(/:(.*?);/) || [])[1] || previewDoc.fileType || "application/octet-stream";
  ...
  const handleDownload = (id) => {
    ...
    if (doc && doc.fileData) {
      try {
        // Convert data URL -> Blob so mobile browsers handle it reliably
        ...
        a.click();
        a.remove();
        // Fallback for iOS Safari, which ignores the download attribute
        setTimeout(() => {
        ...
        setToast("⚠️ Could not download this file.");
      }
    } else {
      setToast("ℹ️ No file attached to this sample document.");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (form.file) {
      ...
      try { fileData = await readFileAsDataURL(form.file); } catch { fileData = null; }
    }
    const newDoc = {
      id: Date.now(),
      title: form.title,
    ...
    try {
      ...
      setToast("⚠️ File too large to store locally.");
      return;
    }
            ...
            <div className="preview-body">
              {previewUrl && previewDoc.fileType === "application/pdf" ? (
                <div style={{ marginBottom: 18 }}>
                  <iframe title="PDF preview" src={previewUrl} style={{ width: "100%", height: "50vh", border: `1px solid ${S.border}`, borderRadius: 12, background: "#fff" }}></iframe>
                  <div style={{ textAlign: "center", marginTop: 8 }}>
                    <a href={previewUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: S.teal, fontWeight: 600 }}>Open full preview ↗</a>
                  </div>
                </div>
              ) : previewUrl && previewDoc.fileType && previewDoc.fileType.startsWith("image/") ? (
                <div style={{ marginBottom: 18, textAlign: "center" }}>
                  <img alt="preview" src={previewUrl} style={{ maxWidth: "100%", maxHeight: "50vh", borderRadius: 12, border: `1px solid ${S.border}` }} />
                </div>
              ) : (
                <div className="preview-pane">
                  <div className="preview-page">
                    <div className="preview-line" style={{ width: "70%", height: 11, background: S.teal, opacity: 0.7 }}></div>
                    <div className="preview-line" style={{ width: "100%" }}></div>
                    <div className="preview-line" style={{ width: "92%" }}></div>
                    <div className="preview-line" style={{ width: "96%" }}></div>
                    <div className="preview-line" style={{ width: "60%" }}></div>
                    <div className="preview-line" style={{ width: "100%", marginTop: 18 }}></div>
                    <div className="preview-line" style={{ width: "85%" }}></div>
                    <div className="preview-line" style={{ width: "90%" }}></div>
                  </div>
                  <div className="preview-note">{previewDoc.fileName ? `${previewDoc.fileName} · preview not available for this file type` : "no file attached · sample document"}</div>
                </div>
              )}
              <div className="preview-desc">{previewDoc.description}</div>
              <div className="preview-meta-row">
                <span className="tag tag-user">↑ {previewDoc.uploader}</span>
                <span className="tag tag-date">{previewDoc.date}</span>
                <span className="tag tag-cat">{previewDoc.downloads} downloads</span>
              </div>
            </div>
