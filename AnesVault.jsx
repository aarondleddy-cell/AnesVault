...
function WriteOffsView({ onToast }) {
  ...
  const generateHTML = () => {
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>AnesVault Business Write-Offs</title><style>
      ...
      <div class="header"><div class="logo">Anes<span>Vault</span></div><div class="subtitle">Business Write-Off Reference Guide</div><div class="date">Generated ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</div></div>
      <div class="disclaimer">⚠️ <strong>Not tax advice.</strong> This is a general educational reference for independent anesthesia providers (CRNAs, MDAs). Always confirm deductions with a licensed CPA or tax professional before filing.</div>
      <div class="total">${totalItems} deductions across ${allGroupsForPDF.length + (miscCommunityPDF.length > 0 ? 1 : 0)} categories</div>
      ${allGroupsForPDF.map(group => `<div class="group"><div class="group-title">${group.group}</div>${group.items.map(item => `<div class="item"><div class="bullet">$</div><div><div class="item-name">${item.name}${item.community ? '<span class="community-badge">community</span>' : ''}</div>${item.note ? `<div class="item-note">${item.note}</div>` : ""}</div></div>`).join("")}</div>`).join("")}
      ${miscCommunityPDF.length > 0 ? `<div class="group"><div class="group-title">Miscellaneous</div>${miscCommunityPDF.map(item => `<div class="item"><div class="bullet">$</div><div><div class="item-name">${item.name}<span class="community-badge">community</span></div>${item.note ? `<div class="item-note">${item.note}</div>` : ""}</div></div>`).join("")}</div>` : ""}
      <div class="footer">AnesVault · anesvault.com · @Anes.Vault on Instagram & TikTok<br/>Built by anesthesia, for anesthesia.</div>
    </body></html>`;
  };
  ...
  const handleDownloadPDF = () => {
    ...
    pdfWindow.onload = () => {
      pdfWindow.document.title = "AnesVault-Business-Write-Offs";
      setTimeout(() => {
        pdfWindow.print();
    ...
    <div className="section-wrap">
      <div className="section-head">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button onClick={handleDownloadPDF} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", background: S.teal, color: "white", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
            <Icon name="download" size={13} /> PDF
          </button>
          <button onClick={handlePrint} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", background: S.slate, color: "white", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
            🖨 Print
          </button>
        </div>
      </div>
    ...
    <div className="hub-root">
      ...
      <div className="nav-section">
        <div className="nav-grid">
          ...
          <a href="https://www.youtube.com/@AnesVault" target="_blank" rel="noreferrer" className="nav-card" style={{ textDecoration: "none" }}>
            <div className="nav-icon" style={{ background: "linear-gradient(135deg, #FF0000, #CC0000)" }}><Icon name="youtube" size={18} /></div>
            <div><div className="nav-label">Educational Resources</div><div className="nav-sub">Watch on YouTube</div></div>
          </a>
        </div>
      </div>
      ...
      <footer className="hub-footer">
        ...
        <div className="footer-links">
          ...
          <a href="mailto:ADLMedgroup@gmail.com?subject=AnesVault%20Inquiry" className="footer-link" style={{ textDecoration: "none" }} onClick={e => { e.preventDefault(); setShowContact(true); }}>Contact</a>
      ...
      {/* ── CONTACT MODAL ── */}
      {showContact && (
        <div className="join-modal-overlay" onClick={() => setShowContact(false)}>
          <div className="join-modal-box" onClick={e => e.stopPropagation()}>
            <button style={{ float: "right", background: "none", border: "none", cursor: "pointer", color: S.slate }} onClick={() => setShowContact(false)}><Icon name="x" size={20} /></button>
            <div className="modal-title">Contact Us</div>
            <div className="modal-sub">Have a question or suggestion? We'll get back to you at ADLMedgroup@gmail.com.</div>
            <div className="form-group">
              <label className="form-label">Your Name</label>
              <input className="form-input" type="text" placeholder="First name" value={contactName} onChange={e => setContactName(e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Your Email</label>
              <input className="form-input" type="email" placeholder="you@email.com" value={contactEmail} onChange={e => setContactEmail(e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Message</label>
              <textarea className="form-textarea" placeholder="What's on your mind?" value={contactMessage} onChange={e => setContactMessage(e.target.value)} style={{ minHeight: 100 }} />
            </div>
            <button className="submit-btn" disabled={!contactEmail || !contactMessage || contactLoading} onClick={async () => {
              ...
              } catch { setToast("❌ Failed to send. Try emailing ADLMedgroup@gmail.com directly."); }
              setContactLoading(false);
            }}>
              <Icon name="mail" size={16} /> {contactLoading ? "Sending…" : "Send Message"}
            </button>
          </div>
        </div>
      )}
      ...
      {showJoin && (
        <div className="join-modal-overlay" onClick={() => setShowJoin(false)}>
          <div className="join-modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              <label className="form-label">Your Name</label>
              <input className="form-input" type="text" placeholder="First name" value={joinName} onChange={e => setJoinName(e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Your Email</label>
              <input className="form-input" type="email" placeholder="you@email.com" value={joinEmail} onChange={e => setJoinEmail(e.target.value)} />
            </div>
            <button className="submit-btn" disabled={!joinEmail || joinLoading} onClick={async () => {
              if (!joinEmail) return;
              setJoinLoading(true);
              try {
                const welcomeHtml = `
                  <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #0D2B3E;">
                    ...
                    <div style="background: #F0F5F7; padding: 32px; border-radius: 0 0 12px 12px;">
                      <p style="font-size: 18px; font-weight: bold; margin-bottom: 12px;">Hey ${joinName || "there"}, welcome to AnesVault! 🎉</p>
              ...
              setShowJoin(false);
              setToast("🎉 Welcome! Check your email for a message from AnesVault.");
              setJoinEmail(""); setJoinName("");
              setJoinLoading(false);
            }}>
              <Icon name="mail" size={16} /> {joinLoading ? "Sending…" : "Join the Community"}
            </button>
          </div>
        </div>
      )}
