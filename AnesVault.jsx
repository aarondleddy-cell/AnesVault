...
const docAPI = {
  ...
  async getPending() {
    const { data, error } = await supabase.from("documents").select("*").eq("approved", false).order("created_at", { ascending: false });
    if (error) { console.error(error); return []; }
    return data || [];
  },
  async insert(doc) {
...
function AnesVault() {
  ...
  const refreshPending = async () => {
    const data = await docAPI.getPending();
    setPending(data);
  };
  ...
  const handleDownload = async (id) => {
    ...
    try {
      await docAPI.update(id, { downloads: (doc.downloads || 0) + 1 });
  ...
  const handleApprove = async (id) => {
    try {
      await docAPI.update(id, { approved: true });
